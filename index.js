...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    document.getElementById('error-message').textContent = 
      "System loading... Please refresh if this persists.";
  }
});

// Save tokens when page is closing
window.addEventListener('beforeunload', async () => {
  if (miningInterval && userRef) {
    const elapsedSeconds = (Date.now() - miningStartTime) / 1000;
    const tokensMined = elapsedSeconds * 0.001;
    ...
    if (tokensMined > 0) {
      try {
        ...
        console.error("Error saving tokens on unload:", error);
      }
    }
  }
});
